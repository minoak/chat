...
## 🔄 개발 워크플로우 (Claude Code 사용 시)

### 브랜치 관리

Claude Code를 GitHub 환경에서 사용하면 각 세션마다 새로운 브랜치가 자동으로 생성됩니다.
효율적인 브랜치 관리를 위한 워크플로우:

#### 작업 중

각 세션에서 자유롭게 작업하고 커밋합니다. Claude Code가 자동으로 브랜치를 생성하고 관리합니다.

#### 작업 완료 후 - 디폴트 브랜치 업데이트

중요한 작업이 완료되면 디폴트 브랜치에 최신 내용을 반영합니다.

**방법 A: Pull Request (권장)**
1. GitHub에서 현재 작업 브랜치 → 디폴트 브랜치로 PR 생성
2. 변경사항 확인 후 Merge
3. 작업 브랜치 삭제 (선택)

**방법 B: GitHub 웹에서 수동 병합**
1. Repository → **Branches** 페이지
2. 최신 작업 브랜치 확인
3. 디폴트 브랜치와 비교 후 병합

#### 주기적 브랜치 정리

오래된 작업 브랜치들을 정리하여 저장소를 깔끔하게 유지:

1. GitHub → **Branches** 페이지 접속
2. `claude/` 접두사가 붙은 오래된 브랜치 확인
3. 병합 완료되었거나 불필요한 브랜치 삭제
4. 디폴트 브랜치와 최신 작업 브랜치만 유지
...
2. Default branch를 최신 안정 버전으로 유지
3. 필요시 브랜치 이름 변경 (예: `main`)

### 로컬 환경 작업 (선택사항)

여러 기기(폰/컴퓨터)에서 작업하지 않는 경우, 로컬 환경에서 브랜치를 직접 제어할 수 있습니다:

```bash
# 리포지토리 클론
git clone https://github.com/minoak/dudada.git
cd dudada
...
# 작업 후 푸시
git push -u origin my-work-branch
```

**주의:** 여러 기기에서 작업 시 동기화 문제가 발생할 수 있으므로 GitHub 환경 권장
